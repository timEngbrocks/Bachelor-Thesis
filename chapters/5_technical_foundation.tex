...
% TODO: Kubernetes Intro
% TODO: Nodes > Pods > Containers
% TODO: What is a Kubernetes Resource
...
% TODO: rewrite
% Helm \cite{HEL-DOCS} is a templating engine for Kubernetes resource definitions.
% Kubernetes does not provide any out-of-the-box support for dynamic properties.
% Helm resolves this issue by creating template files with dynamic properties,
% which user-controlled values can replace. Helm then fills the dynamic properties
% with these values and renders them into a standard Kubernetes resource definition file.
% This mechanism enables, for example, the creation of multiple deployments from the same definition but with different names.
% Under the hood, Helm uses the Go language to parse the templates, which allows additional functionality,
% like flow control and adapting provided values. Helm combines resources that should be templated into what is called a Helm Chart.
% Each Chart has a single configuration file that contains the values used for templating the resources in the Helm Chart.
